...
- **Handling NULL Values**: To ensure data consistency, missing values in the age column were imputed based on the average age of customers within the same product category.
- **Fixing Inconsistencies**: Standardized the category names. 
...
1. **Write a SQL query to retrieve the most profitable category**:
```sql
SELECT 
    category,
...
```
2. **Write a SQL query to retrieve the busy time for shopping**:
```sql
SELECT 
...
GROUP BY shift
```
13. **Write a SQL query to find the Total sales based on Age(kids: 0-15, Teen: 16-28, Mid_age: 29-45, Old_age: +45)**:
```sql
SELECT 
    CASE 
        WHEN age BETWEEN 0 AND 15 THEN 'Kids'
        WHEN age BETWEEN 16 AND 28 THEN 'Teen'
        WHEN age BETWEEN 29 AND 45 THEN 'Mid_age'
        ELSE 'old_age'
    END AS age_group,
...
2. **Set Up the Database and Run the Queries**: Run the SQL scripts provided in the `sql_query_p1.sql` file to create, populate the database and to perform your analysis.
...
## Contact
**Rahul Durga Ganesh Ummidi**
**Email**: rahulummidi789@gmail.com
**LInkedIn**: www.linkedin.com/in/rahul-ummidi
